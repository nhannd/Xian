...
{
	[TestFixture]
	public class StudyXmlReaderTests : AbstractTest
	{
        ...
        public void TestFromXml()
        {
            ...
            var reader = new ReaderWrapper(test);
            studyXml.SetMemento(new ReaderNode(reader));
            Assert.AreEqual(29, studyXml.NumberOfStudyRelatedSeries);
            
        }

        [Test]
        public void TestSopClassReader()
        {
            ...
            var reader = new ReaderWrapper(sw.ToString());
            xml.SetMemento(new ReaderNode(reader));
		...
		public void TestChangeTransferSyntax()
		{
            ...
            doc.WriteTo(tx);
            // i use literal control because when you set its mode to "encode", you can write xml tags to browser window as they are
            reader = new ReaderWrapper(sw.ToString());
            xml.SetMemento(new ReaderNode(reader));
		...
		public void TestSameSequenceWithExclusionsMultipleSerializations()
		{
            ...
            // i use literal control because when you set its mode to "encode", you can write xml tags to browser window as they are
            var reader = new ReaderWrapper(sw.ToString());
            newStudyXml.SetMemento(new ReaderNode(reader));
