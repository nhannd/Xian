...
{
    public class AttributeSS : AbstractAttribute
    {
        private short[] _values = null;
        ...
        internal AttributeSS(DicomTag tag, OffisDcmItem item)
            : base(tag)
        {
            ...
            OFCondition status = item.Item.findAndGetSint16Array(offisTag, ref array, ref length);
        ...
        internal AttributeSS(AttributeSS attrib)
            : base(attrib)
        {
            short[] values = (short[])attrib.Values;
        ...
        public override bool Equals(object obj)
        {
            ...
            AttributeSS a = (AttributeSS)obj;
            short[] array = (short[])a.Values;
        ...
        public override void SetStringValue(String stringValue)
        {
            ...
            _values = new short[Count];

            for (int i = 0; i < stringValues.Length; i++)
            {
                if (!short.TryParse(stringValues[i], out _values[i]))
                    throw new DicomException(SR.InvalidStringFormat);
            }
        ...
        internal override void FlushAttribute(OffisDcmItem item)
        {
            if (base.Dirty)
            {
                ...
                for (int i = 0; i < _values.Length; i++)
                    item.Item.putAndInsertSint16(offisTag, _values[i], (uint)i);
